...
function lcms_add_experience_page_scripts_styles()
{
...
add_action('wp_print_styles', 'lcms_add_experience_page_scripts_styles');
add_action('wp_print_scripts', 'lcms_add_experience_page_scripts_styles');
...
add_shortcode('lcms_add_experience_page', 'lcms_add_experience_page_shortcode');
function lcms_add_experience_page_shortcode($atts)
{
	wp_print_scripts();
	wp_print_styles();
	...
	$message = '<p>' . __('You do not have the correct membership to add a client experience.', 'lcms-geodirectory-custom-functions') . '</p>';
	...
	if ($_POST['submit_button'] && $_POST['submit_button'] == 'save_experience') {
		...
		if ($sqlexecute) {
			...
			lcms_send_email_admin_new_experience_approval(
				$new_post_id,
				$user_name,
				$user_email,
				$user_user_id,
	...
	if (
		...
		($membership_level[1] == 'other' && lcms_count_user_experience_records($current_user_id) == 0) ||
		($membership_level[1] == 'other' && lcms_count_user_experience_records($current_user_id) > 1)
	) {
		...
		$html = '
					<div id="my-client-experience-container" style="margin: 2rem 0;">
						<h1 style="font-size:2rem;color: var(--ast-global-color-2); font-weight: 500;">' . __('Share Your Experience', 'lcms-geodirectory-custom-functions') . '</h1>

						<p style="margin-bottom:30px;">' . __('Add the details of your client experience in two simple steps.', 'lcms-geodirectory-custom-functions') . '</p>
...
<div class="client-input-error" style=""><p style="">Something Went Wrong!</p></div>';
		...
		$html .= '<!-- jQuery script for below Nav Tabs, next/prev button click event actions -->
						...
						<div class="tab-content">
							...
							<!--tab pane -->
							<div class="tab-pane container active client-title" id="client-details">
								<h2 style="font-size:1.3rem;" class="mt-4">' . __('Your Client\'s Details', 'lcms-geodirectory-custom-functions') . '</h2>
									<!--<div class="form-group row mt-2 mb-2">
										<label for="exp_add_client_category" class="col-sm-3 col-form-label" >' . __('Client Category', 'lcms-geodirectory-custom-functions') . '<span class="text-danger">*</span></label>
										<select class="col-sm-4" name="exp_add_client_category" id="exp_add_client_category" required>
											<option value="0" selected disabled>' . __('Select', 'lcms-geodirectory-custom-functions') . '</option>';
		...
		$html .= '</select>
			...
			<div class="row experience-form">
				<div class="col-sm-6">
					<div class="form-group row mt-2" style="margin-right:20px;margin-left: 0px;">
						<label for="exp_add_client_name" class="col-form-label" >' . __('Client\'s Name', 'lcms-geodirectory-custom-functions') . '<span class="text-danger">*</span></label>
						<input class="" type="text" name="exp_add_client_name" id="exp_add_client_name" class="form-control" required>
					</div>
				</div>
				<div class="col-sm-6">
					<div class="form-group row mt-2" style="margin-right:20px;">
						<label for="exp_add_client_phone_number" class="col-form-label" >' . __('Client\'s Phone Number', 'lcms-geodirectory-custom-functions') . '</label>
						<input class="" type="text" name="exp_add_client_phone_number" id="exp_add_client_phone_number" class="form-control">
					</div>
				</div>			
			</div>
			<div class="row experience-form">
				<div class="col-sm-6">
					<div class="form-group row" style="margin-right:20px;margin-left: 0px;">
						<label for="exp_add_client_email" class="col-form-label" >' . __('Client\'s Email', 'lcms-geodirectory-custom-functions') . '<span class="text-danger">*</span></label>
						<input class="" type="email" name="exp_add_client_email" id="exp_add_client_email" class="form-control" required>
					</div>
				</div>
				<div class="col-sm-6">
					<div class="form-group row" style="margin-right:20px;">
						<label for="exp_add_client_zipcode" class="col-form-label" >' . __('Client\'s Zip Code', 'lcms-geodirectory-custom-functions') . '<span class="text-danger">*</span></label>
						<input class="" type="text" name="exp_add_client_zipcode" id="exp_add_client_zipcode" class="form-control" required>
					</div>
				</div>
			</div>
			<div style="width:100%">
				<div id="add-experience-button" class="experience-form-btn">
					<a class="btn btn-primary btnNext mt-5">' . __('Next', 'lcms-geodirectory-custom-functions') . '</a>
				</div>
			</div>
							...
							<div class="tab-pane container fade" id="experience">
								<h2 style="font-size:1.3rem;" class="mt-4">' . __('Your Experience', 'lcms-geodirectory-custom-functions') . '</h2>
								<div class="form-group row mt-2 mb-2">
									<div class="col-md-8">
										<label for="exp_add_client_experience" class="col-form-label" >' . __('Describe your experience with the client', 'lcms-geodirectory-custom-functions') . '<span class="text-danger">*</span></label>
										<textarea class="" rows="8" name="exp_add_client_experience" minlength="100" maxlength="2000" id="exp_add_client_experience" class="form-control" required></textarea>
									</div>
		...
		<div class="col-md-4 exp_add_client_attachement_div">
			<h2 style="font-size:1.3rem;" class="mt-4">' . __('Upload Attachment (Optional)', 'lcms-geodirectory-custom-functions') . '</h2>
		...
		<a class="btn btn-primary exp_add_client_attachement_addon mt-5">' . __('Add New Attachment', 'lcms-geodirectory-custom-functions') . '</a>

								<!-- NEW FILE UPLOAD SECTION END-->
									
								</div>

								<div><a class="btn btn-primary btnPrevious mt-5">' . __('Back', 'lcms-geodirectory-custom-functions') . '</a>
								<button class="btn bsui btn-primary" name="submit_button" value="save_experience">' . __('Submit Client', 'lcms-geodirectory-custom-functions') . '</button></div>
							</div> <!--tab pane -->
					...
					';
	} elseif ($membership_level[1] == 'other' && lcms_count_user_experience_records($current_user_id) == 1) {
		// If user is not a member (yet) and the free one-time client experience has already been entered by user
		$html = '
					<div style="margin: 2rem 0;">
						<h1 style="font-size:1.75rem;margin-bottom:20px;color: var(--ast-global-color-2); font-weight: 500;">' . __('List a Client', 'lcms-geodirectory-custom-functions') . '</h1>
						<p style="margin-top:15px;">' . __('Note: You have already used your one-time client experience.', 'lcms-geodirectory-custom-functions') . '</p>
					</div>
					';
