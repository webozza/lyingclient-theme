jQuery(document).ready(function ($) {
  $(".tab-pane#client-details input, .tab-pane#experience input")
    .parent()
    .append('<p style="color:red" class="webozza-error-msg"></p>');

  function errorMsg() {
    const client_input_error = $(".client-input-error");
    const exp_add_client_name = jQuery(
      'input[name="exp_add_client_name"]'
    ).val();
    const exp_add_client_phone_number = jQuery(
      'input[name="exp_add_client_phone_number"]'
    ).val();
    const exp_add_client_email = jQuery(
      'input[name="exp_add_client_email"]'
    ).val();
    const exp_add_client_zipcode = jQuery(
      'input[name="exp_add_client_zipcode"]'
    ).val();
    var re = /\S+@\S+\.\S+/;
    if (exp_add_client_name == "") {
      $("input#exp_add_client_name")
        .parent()
        .find(".webozza-error-msg")
        .text("Client Name Field Is Missing! Try again");
    }
    if (exp_add_client_email == "") {
      $("input#exp_add_client_email")
        .parent()
        .find(".webozza-error-msg")
        .text("Client Email Field Is Missing! Try again");
    }
    if (!re.test(exp_add_client_email)) {
      $("input#exp_add_client_email")
        .parent()
        .find(".webozza-error-msg")
        .text("Please Enter Valid E-mail! Try again");
    }
    if (exp_add_client_zipcode == "") {
      $("input#exp_add_client_zipcode")
        .parent()
        .find(".webozza-error-msg")
        .text("Client zip Code Field Is Missing! Try again");
    }
    if (isNaN(exp_add_client_zipcode)) {
      $("input#exp_add_client_zipcode")
        .parent()
        .find(".webozza-error-msg")
        .text("Please Enter valid Client Zip Code! Try again");
    }
  }
  ...
  function FieldsChecker(stepTwo) {
    ...
    if (
      exp_add_client_name != "" &&
      exp_add_client_email != "" &&
      re.test(exp_add_client_email) &&
      exp_add_client_zipcode != "" &&
      !isNaN(exp_add_client_zipcode)
    ) {
      ...
      if (stepTwo) {
        $("ul.nav-tabs .step-two-prevent a").attr(
          "style",
          "pointer-events: auto"
        );
      }
      ...
      if (exp_add_client_name == "") {
        client_input_error.html(
          "<p>Client Name Field Is Missing! Try again</p>"
        );
        $("input#exp_add_client_name")
          .parent()
          .find(".webozza-error-msg")
        ...
        client_input_error.html(
          "<p>Client Email Field Is Missing! Try again</p>"
        );
        $("input#exp_add_client_email")
          .parent()
          .find(".webozza-error-msg")
        ...
        client_input_error.html(
          "<p>Client zip Code Field Is Missing! Try again</p>"
        );
        $("input#exp_add_client_zipcode")
          .parent()
          .find(".webozza-error-msg")
        ...
        client_input_error.html(
          "<p>Please Enter valid Client Zip Code! Try again</p>"
        );
        $("input#exp_add_client_zipcode")
          .parent()
          .find(".webozza-error-msg")
  ...
  function attachementChangeFunction() {
    jQuery(
      '.exp_add_client_attachement_div input[name="exp_add_client_attachement[]"]'
    ).change((e) => {
      const blob_url = URL.createObjectURL(e.target.files[0]);
      const parent_div_parent = jQuery(e.currentTarget).parents(
        ".exp_add_client_attachement_parent_div"
      );
      console.log("s");
      parent_div_parent
        .children(".exp_add_client_attachement_parent_preview")
        .addClass("active");
      parent_div_parent
        .children(".exp_add_client_attachement_parent_preview")
        .attr("src", blob_url);
  ...
  function attachementRemoveFunction() {
    jQuery("a.exp_add_client_attachement_remove_child").click((e) => {
      ...
      jQuery(e.currentTarget)
        .parents(".exp_add_client_attachement_parent_div")
        .remove();
    });
  }
  attachementRemoveFunction();
  ...
  $("select#exp_add_client_category").change(function () {
    var selectedVal = $("select#exp_add_client_category")
      .find(":selected")
      .val();
    if (!selectedVal == 0) {
  ...
  $("#my-client-experience-container form button.bsui.btn-primary").on(
    "click",
    function () {
      ...
      if ($("select#exp_add_client_category").find(":selected").val() == 0) {
        $("#exp_add_client_category")
          .parent()
          .append(
            '<p style="color:red" class="webozza-error-msg">Please Select A Value</p>'
          );
      }
    }
  );
