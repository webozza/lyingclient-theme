jQuery(document).ready(function ($) {
  ...
  var lcms_ajax_url = jQuery('input[name="admin_ajax_url_lcms"]').val();
  ...
  jQuery("a.exp_edit_client_attachement_addon").click((e) => {
    ...
    jQuery("div.exp_edit_client_attachement_div").append(
      '<div class="exp_edit_client_attachement_parent_div"><img src="none" class="exp_edit_client_attachement_image_preview" height="250" width="250" style="border: 2px solid red; padding: 5px; margin: 5px"/><input type="file" accept="image/png, image/jpeg" name="exp_edit_client_attachement[]" id="exp_edit_client_attachement" class="form-control" /><input type="hidden" name="exp_edit_client_attachement_input[]" id="exp_edit_client_attachement_input" class="form-control" /><a href="#" class="btn exp_edit_client_attachement_remove_child">Remove</a></div>'
    );
  ...
  function attachementEditRemoveFunction() {
    jQuery("a.exp_edit_client_attachement_remove_child").click((e) => {
      ...
      jQuery(e.currentTarget)
        .parents(".exp_edit_client_attachement_parent_div")
        .remove();
    });
    ...
    jQuery('input[name="exp_edit_client_attachement[]"]').change((e) => {
      ...
      const parent_div_parent = jQuery(e.currentTarget).parents(
        ".exp_edit_client_attachement_parent_div"
      );

      parent_div_parent.prepend(
        '<p class="exp_edit_client_attachement_loading_state">your image is uploading!...</p>'
      );

      if (
        parent_div_parent
          .children(".exp_edit_client_attachement_image_preview")
          .hasClass("active")
      ) {
        parent_div_parent
          .children(".exp_edit_client_attachement_image_preview")
      ...
      jQuery.ajax({
        ...
        success: function (data) {
          ...
          parent_div_parent
            ...
            .addClass("active");
          parent_div_parent
            .children(".exp_edit_client_attachement_loading_state")
            .remove();

          parent_div_parent
            .children('input[name="exp_edit_client_attachement_input[]"]')
            .attr("value", data);
        },
