...
//wp_enqueue_style('bootstrap5-dbase', esc_url( plugin_dir_url( __FILE__ ) . '../../libraries/bootstrap/css/bootstrap.min.css' )); // Bootstrap 5 css - in header
...
function dbase_check_if_logged_in() {
    if ( !is_user_logged_in() ) {
        return array(
            'login_status' => false,
            'login_status_message' => '<div style="margin-top:30px;">'.
            '<p>'.__( 'You need to be logged in.','d-base-geodirectory-custom-functions').'</p>
            <p><button class="geodir_button btn btn-primary dbase-button" name="login_button"><a style="text-decoration: none;color:#fff;" href="'.get_site_url().'/login">' . __( 'Login','d-base-geodirectory-custom-functions' ) . '</a></button></p>'.
            '</div>'
        );
    } else {
        // Get user ID
...
function dbase_check_membership_level( $message ) {
	// MEMBERSHIP CHECK
    ...
    $upgrade_message =  '<div style="margin: 2rem 0;">'.
							'<h2 style="font-size:1.75rem;margin-bottom:20px;color: var(--ast-global-color-2); font-weight: 500;">'.__('Client Experience','d-base-geodirectory-custom-functions').'</h2>'.
							'<p style="font-size:0.8rem;margin-top:15px;">'.__( $message,'d-base-geodirectory-custom-functions').'</p>'.
...
function dbase_check_membership_level_with_user_id( $user_id ) {
	// MEMBERSHIP CHECK
	// Resource: https://www.paidmembershipspro.com/documentation/content-controls/require-membership-function/#
...
function dbase_count_user_experience_records( $user_id ) {
...
function dbase_send_email_admin_new_experience_approval(
...
// 	$to = get_option('admin_email');
	...
	$headers[] = 'Bcc: mario@d-base.co.za';	// FOR DEBUGGING ONLY
