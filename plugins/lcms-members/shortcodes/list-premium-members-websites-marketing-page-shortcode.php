...
add_shortcode('dbase_list_members_websites_marketing', 'dbase_list_members_websites_marketing_shortcode'); 
function dbase_list_members_websites_marketing_shortcode( $atts ) {
...
*/
	...
	/** START THE HTML CREATION **/
		...
		$html .= '<div style="margin: 2rem 0;">
					<h2 style="font-size:1.75rem;color: var(--ast-global-color-2); font-weight: 500;">'.__('Premium Member Club','d-base-geodirectory-custom-functions').'</h2>
					<p style="">'.__('A directory of our Premium members business websites. Please contact them directly for any service request.','d-base-geodirectory-custom-functions').'</p>
				</div>';
				...
				if ($wpdb->num_rows > 0) {
					...
					$html .= '<div style="margin: 1rem 0 3rem 0;">';
									...
									$html .= '  		<option value="0" selected disabled>'.__('select category','d-base-geodirectory-custom-functions').'</option>';
									...
									//foreach ( $categories_array as $category_row ) {
										...
										$html .= '  		<option value="'.$categories_array[$count]['category_slug'].'">'.__($categories_array[$count]['category_name'],'d-base-geodirectory-custom-functions').'</option>';
										//$count ++;
									}
					...
					if ( $_POST['submit_button_category'] && sanitize_text_field($_POST['member_categories']) ) {
						$html .= '<p>'.__('Business category','d-base-geodirectory-custom-functions').' <strong>"'.$category_name.'"</strong>:</p>';
					}
					...
					$html .= '<ul>';
						...
						foreach ( $users_updated_array as $search_row ) {
							...
							$membership_level_for_user_id = dbase_check_membership_level_with_user_id( $search_row->user_ID );
					...
					$html.= '<p>'.__('No Premium member listings found.<br>'.$current_user_id,'d-base-geodirectory-custom-functions').'</p>';
				}
