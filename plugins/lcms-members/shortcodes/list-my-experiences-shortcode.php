...
add_shortcode('dbase_list_my_client_experiences', 'dbase_list_my_client_experiences_shortcode'); 
function dbase_list_my_client_experiences_shortcode( $atts ) {
	...
	/** Check MEMBERSHIP LEVEL **/
		//$message = '<p>'.__('You do not have the correct membership to display client experiences.','d-base-geodirectory-custom-functions').'</p>';
		...
		// $message = '<p>'.__('You do not have the correct membership to display client experiences.','d-base-geodirectory-custom-functions').'</p>';
		// $membership_level = dbase_check_membership_level( $message );
	...
	// FIND ALL RECORDS FOR MY EXPERIENCES
		...
		$html .= '
				<div style="margin: 2rem 0;">
					<h2 style="font-size:1.75rem;color: var(--ast-global-color-2); font-weight: 500;">'.__('My Client Experiences','d-base-geodirectory-custom-functions').'</h2>
					<p style="">'.__('My listed experiences','d-base-geodirectory-custom-functions').'</p>
				</div>

				<div style="margin: 1rem 0;">
					<ul>';
						if ($wpdb->num_rows > 0) {
							//$html .= '<p>'.__('My experiences','d-base-geodirectory-custom-functions').':</p>';
							foreach ( $search_results as $search_row ) {
								//$html.= '<li><a href="'.esc_url(get_home_url()).'/client-experience/?experience_slug='.str_replace(' ', '-', $search_row->_search_title).'">'.$search_row->post_title.' - '.__('Complaint by:','d-base-geodirectory-custom-functions').' '.$search_row->business_name.'</a></li>';
							...
							$html.= '<p>'.__('No client experience listings found for your account.','d-base-geodirectory-custom-functions').'</p>';
						}
		$html .= '	</ul>
				</div>';
