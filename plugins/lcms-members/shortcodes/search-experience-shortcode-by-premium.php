...
add_shortcode('dbase_premium_search', 'dbase_premium_search_shortcode');
function dbase_premium_search_shortcode($atts)
{
    wp_print_styles();
    wp_print_scripts();
    ...
    if ($a['category'] == 'uncategorized' && !isset($_POST['place_categories'])) {
        $html = '<div style="margin: 2rem 0;">
						<h1 style="font-size:1.75rem;margin-bottom:20px;color: var(--ast-global-color-2); font-weight: 500;">' . __('Premium Member Club', 'd-base-geodirectory-custom-functions') . '</h1>
						...
						<p style="font-size:0.8rem;">' . __('You can Search by using a zip code and city/state.', 'd-base-geodirectory-custom-functions') . '</p>
...
//test end
    ...
    if (isset($_POST['place_categories'])) {
		...
		$html .= '<h1 style="font-size:1.6rem;margin-bottom:20px;">' . __('Business Category', 'd-base-geodirectory-custom-functions') . ': <strong data-cat-id="'. $search_term .'">"' . __(get_term($search_term)->name, 'd-base-geodirectory-custom-functions') . '"</strong></h1>';
...
add_shortcode('dbase_category_premium_dropdown', 'dbase_category_premium_dropdown_shortcode');

function dbase_category_premium_dropdown_shortcode($atts)
{
    // Things that you want to do. 
    $a = shortcode_atts(array(
        'category' => 'uncategorized',
