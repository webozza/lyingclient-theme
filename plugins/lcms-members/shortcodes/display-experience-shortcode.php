...
function dbase_display_experience_page_scripts_styles()
{
...
add_action('wp_print_styles', 'dbase_display_experience_page_scripts_styles');
add_action('wp_print_scripts', 'dbase_display_experience_page_scripts_styles');

/*
 * DISPLAY CLIENT EXPERIENCE - shortcode
 */

add_shortcode('dbase_display_experience_page', 'dbase_display_experience_page_shortcode');
function dbase_display_experience_page_shortcode($atts)
{
	wp_print_styles();
	wp_print_scripts();
	...
	// $message = '<p>' . __('You do not have the correct membership to display this client experience.', 'd-base-geodirectory-custom-functions') . '</p>';
	...
	if (!isset($_GET['experience_slug'])) {
		return
			...
			'<p style="margin-top:15px;">' . __('No client experience was selected.', 'd-base-geodirectory-custom-functions') . '</p>' .
			'</div>';
	}
	...
	if (!empty($client_experience)) {
		$html = '
					<div style="margin: 2rem 0;">
						<h1 style="font-size:2rem;color: var(--ast-global-color-2); font-weight: 500;">' . __('Client Experience', 'd-base-geodirectory-custom-functions') . '</h1>
						<p style="margin-bottom:30px;">' . __('The details of the client experience.', 'd-base-geodirectory-custom-functions') . '</p>';
							...
							<div class="tab-pane container active" id="client-experience-details">
								...
								<div class="row">
									<div class="col-md-6 col-sm-12">
										...
										<h2 style="font-size:1.3rem;margin-top:30px;margin-bottom:20px;"><strong>' . __('Member Details', 'd-base-geodirectory-custom-functions') . '</strong></h2>
										<div class="row">
											<p style="margin-bottom:0;font-weight:600;" class="col-sm-5" >' . __('Member\'s Name:', 'd-base-geodirectory-custom-functions') . '</p>
											<p style="margin-bottom:0;" class="col-sm-7" name="exp_display_member_name" id="exp_display_member_name">' . $member_name_datechecked . '</p>
										</div>
										<div class="row">
											<p style="margin-bottom:0;font-weight:600;" class="col-sm-5" >' . __('Member\'s Business:', 'd-base-geodirectory-custom-functions') . '</p>
		...
		$categories_results = dbase_get_categories();	// get all categories
		...
		$html .= '
										<div class="row">
											<p style="margin-bottom:0;font-weight:600;" class="col-sm-5" >' . __('Member\'s Category:', 'd-base-geodirectory-custom-functions') . '</p>
		...
		if ($exists_member_category_name === TRUE) {
			$html .= __($member_category_row->name, 'd-base-geodirectory-custom-functions');	// there is a member category
		} else {
			$html .= __('none', 'd-base-geodirectory-custom-functions');	// there is no member category
		}
		...
		if ($member_website_datechecked) {	// ONLY PREMIUM MEMBERS WEBSITE SHOWS
			...
			$html .= '
										<div class="row">
											<p style="margin-bottom:0;font-weight:600;" class="col-sm-5" >' . __('Member\'s Website:', 'd-base-geodirectory-custom-functions') . '</p>
		...
		$html .= '</div>
									...
									<div class="col-md-6 col-sm-12">
										<h2 style="font-size:1.3rem;margin-top:30px;margin-bottom:20px;"><strong>' . __('Client Details', 'd-base-geodirectory-custom-functions') . '</strong></h2>
											<div class="row">
												<p style="margin-bottom:0;font-weight:600;" class="col-sm-5" >' . __('Client\'s Name', 'd-base-geodirectory-custom-functions') . ':</p>
											...
											<div class="row">
												<p style="margin-bottom:0;font-weight:600;" class="col-sm-5" >' . __('Client\'s Phone Number', 'd-base-geodirectory-custom-functions') . ':</p>
											...
											<div class="row">
												<p style="margin-bottom:0;font-weight:600;" class="col-sm-5" >' . __('Client\'s Email', 'd-base-geodirectory-custom-functions') . ':</p>
											...
											<div class="row">
												<p style="margin-bottom:0;font-weight:600;" class="col-sm-5" >' . __('Client\'s Zip Code', 'd-base-geodirectory-custom-functions') . ':</p>
											...
											<div class="row">
												<p style="margin-bottom:0;font-weight:600;" class="col-sm-5" >' . __('Category', 'd-base-geodirectory-custom-functions') . ':</p>
											...
											<div class="row mt-3">
												<!--<p style="margin-bottom:0;font-weight:600;" class="col-sm-5" >' . __('Rating by member', 'd-base-geodirectory-custom-functions') . ':</p>
												<p style="margin-bottom:0;" class="col-sm-7">' . $client_experience[0]->clients_experience_rating . '-->
													...
													<p style="margin-bottom:0;font-weight:600;" class="col-sm-5" name="exp_display_client_experience_rating" id="exp_display_client_experience_rating">' . __('Rating by Member', 'd-base-geodirectory-custom-functions') . ':</p>
		...
		$html .= '
								...
								<div class="row mt-4">
								<h2 style="font-size:1.3rem;margin-top:30px;margin-bottom:20px;" class="col-sm-12" ><strong>' . __('Experience with Client', 'd-base-geodirectory-custom-functions') . '</strong></h2>
								</div>
								<div class="row">';
		$modified_client_experience = str_replace(
			'Client review:',
			'<strong>' . __('Client review', 'd-base-geodirectory-custom-functions') . ':</strong>',
			$client_experience[0]->clients_experience
		);
		$modified_client_experience = str_replace(
			'Business response:',
			'<strong>' . __('Business response', 'd-base-geodirectory-custom-functions') . ':</strong>',
			$modified_client_experience
		);

		$html .= '<p class="col-sm-12 mb-5" name="exp_display_client_experience" id="exp_display_client_experience">' . __(nl2br($modified_client_experience), 'd-base-geodirectory-custom-functions') . '</p>
								</div>';
		...
		if (!$attachment_show_url = '/wp-content/plugins/d-base-geodirectory-custom-functions/shortcodes/assets/no.png') {
			$html .= '<hr>
		...
		/*if ( $current_user_id == $client_experience_check[0]->post_author && ( $membership_level == 'premium' || $membership_level == 'plus' ) ) {
									$html .= '<a class="btn btn-primary btnNext">'.__('Next','d-base-geodirectory-custom-functions').'</a>';
								}*/
		...
		if ($current_user_id == $client_experience_check[0]->post_author && ($membership_level == 'premium' || $membership_level == 'plus')) {
			...
			$html .= '<!--tab pane -->
								<div class="tab-pane container fade" id="edit-client-experience">
									<h2 style="font-size:1.3rem;margin-top:30px;"><strong>' . __('Edit Client Experience', 'd-base-geodirectory-custom-functions') . '</strong></h2>
									<div class="form-group row mt-2 mb-2">
										<p style="margin-bottom:30px;">' . __('Edit the details of your client experience.', 'd-base-geodirectory-custom-functions') . '</p>
										<form method="post" enctype="multipart/form-data">
										<input type="hidden" name="experience_post_id" value="' . $experience_post_id . '" />
										<input type="hidden" name="admin_ajax_url_dbase" value="' . admin_url('admin-ajax.php') . '" />
											<div class="form-group row mt-2 mb-2">
												<label for="exp_edit_client_name" class="col-sm-3 col-form-label" >' . __('Client\'s name', 'd-base-geodirectory-custom-functions') . '<span class="text-danger">*</span></label>
											...
											<div class="form-group row mt-2 mb-2">
												<label for="exp_edit_client_phone_number" class="col-sm-3 col-form-label" >' . __('Client\'s Phone Number', 'd-base-geodirectory-custom-functions') . '</label>
											...
											<div class="form-group row mt-2 mb-2">
												<label for="exp_edit_client_email" class="col-sm-3 col-form-label" >' . __('Client\'s Email', 'd-base-geodirectory-custom-functions') . '<span class="text-danger">*</span></label>
											...
											<div class="form-group row mt-2 mb-2">
												<label for="exp_edit_client_zipcode" class="col-sm-3 col-form-label" >' . __('Client\'s Zip Code', 'd-base-geodirectory-custom-functions') . '<span class="text-danger">*</span></label>
			...
			$html .= '<div class="form-group row mt-2 mb-2">
												<label for="exp_edit_client_category" class="col-sm-3 col-form-label" >' . __('Client Category', 'd-base-geodirectory-custom-functions') . '<span class="text-danger">*</span></label>
												<select class="col-sm-4" name="exp_edit_client_category" id="exp_edit_client_category" required value="' . $client_experience[0]->default_category . '">
													...
													<option value="' . $client_experience[0]->default_category . '" selected>' . __($client_experience[0]->client_category_name, 'd-base-geodirectory-custom-functions') . '</option>';
			...
			$html .= '</select>
											...
											<div class="form-group row mt-2 mb-2">
												<label for="exp_edit_client_experience" class="col-sm-3 col-form-label" >' . __('My experience with Client', 'd-base-geodirectory-custom-functions') . '<span class="text-danger">*</span></label>
												<textarea class="col-sm-12" rows="8" name="exp_edit_client_experience" id="exp_edit_client_experience" class="form-control" required>' . __($client_experience[0]->clients_experience, 'd-base-geodirectory-custom-functions') . '</textarea>
											</div>

											<div class="form-group row mt-4 mb-2">
												<label for="exp_edit_client_attachement" class="col-sm-3 col-form-label" >' . __('Attachment', 'd-base-geodirectory-custom-functions') . '</label>

												' . $attachement_div_edit_html . '
												
												<a class="btn btn-primary exp_edit_client_attachement_addon mt-5">' . __('Add New Attachment', 'd-base-geodirectory-custom-functions') . '</a>

											</div>

											<!--<div class="form-group row mt-2 mb-2">
												<label for="exp_edit_client_experience_rating" class="col-sm-3 col-form-label" >' . __('Client\'s Experience Rating', 'd-base-geodirectory-custom-functions') . '<span class="text-danger">*</span></label>
												...
												<!--<div class="col-md-4">
												<h2 style="font-size:1.3rem;">' . __('Rate this experience', 'd-base-geodirectory-custom-functions') . '</h2>
											...
											<p style="margin-top:30px;clear:left;"><button class="btn bsui btn-primary" name="submit_button" value="save_experience">' . __('Save changes', 'd-base-geodirectory-custom-functions') . '</button></p>
											
										</form>
						
									</div>
									<!--<a class="btn btn-primary btnPrevious">' . __('Back', 'd-base-geodirectory-custom-functions') . '</a>-->
