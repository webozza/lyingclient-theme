<?php
/**
 * Plugin Name:       LC Masters Solutions
 ...
 * Text Domain:       d-base-geodirectory-custom-functions
...
if (version_compare(PHP_VERSION, '7.0.0', '<')) {
    ...
    __e('D-Base Solutions plugin: This website\'s PHP version is too low. Minimum PHP version 7.0 required');
    return;
}
...
if ( session_status() == PHP_SESSION_NONE ) {
	...
	/*
    //add_action( 'show_session_data', 'dbase_show_session_data' );
    function dbase_show_session_data() {
...
// INCLUDE THE D-BASE FUNCTIONS TOOLBOX
//include 'd-base-toolbox.php';
...
add_action ('wp_enqueue_scripts', 'dbase_enqueue_custom_head_footer_scripts_styles');
function dbase_enqueue_custom_head_footer_scripts_styles() {
	...
	// HEAD SCRIPTS HERE:
		...
		wp_enqueue_style('custom-dbase', plugin_dir_url(__FILE__) . 'css/'. 'custom.css');    // D-Base custom css
		//wp_enqueue_style('animate-dbase', 'https://cdnjs.cloudflare.com/ajax/libs/animate.css/4.1.0/animate.min.css');    // animate elements you select on web page
		//wp_enqueue_style('fancybox-dbase', esc_url( get_stylesheet_directory_uri() . '/3rdparty/fancybox/jquery.fancybox.min.css' )); // lightbox/popup library
		//wp_enqueue_style('aos-dbase', esc_url( get_stylesheet_directory_uri() . '/3rdpartylibraries/aos/dist/aos.css' )); // aos - animate on scroll library
		
		//if ( is_page( array('edit-client-experience-post') ) ) { wp_enqueue_style('bootstrap5-dbase', esc_url( plugin_dir_url( __FILE__ ) . 'libraries/bootstrap/css/bootstrap.min.css' )); } // Bootstrap 5 - ONLY SLECIFIC PAGE(S)
	...
	//FOOTER SCRIPTS HERE:
		...
		// FontAwesome library
		//wp_enqueue_script('fontawesome-dbase', 'https://kit.fontawesome.com/cd5b551998.js', '', '', true);    // icons library

		// Fancybox library and my custom script
		//wp_enqueue_script('fancybox-dbase', esc_url( get_stylesheet_directory_uri() . '/3rdparty/fancybox/jquery.fancybox.min.js' ), array(), '', true);  // lightbox/popup library
		//wp_enqueue_script('custom-fancybox-dbase', esc_url( get_stylesheet_directory_uri() . '/3rdparty/_my-custom-scripts/fancybox/custom_fancybox.js' ), array('fancybox-dbase', 'jquery'), '', true);  // fancybox my custom script

		// AOS library and my custom script
		//wp_enqueue_script('aos-dbase', esc_url( get_stylesheet_directory_uri() . '/3rdpartylibraries/aos/dist/aos.js' ), array('jquery'), '', true);  // aos - animate on scroll library
		//wp_enqueue_script('custom-aos-dbase', esc_url( get_stylesheet_directory_uri() . '/js/dbase-custom.js' ), array('aos-dbase'), '', true);   // fancybox my custom script

		// Particles JS script
		//wp_enqueue_script('particles-dbase', esc_url( get_stylesheet_directory_uri() . '/js/particles/particles.min.js' ), array(), '', true);
		//wp_enqueue_script('particles-app-dbase', esc_url( get_stylesheet_directory_uri() . '/js/particles/app.js' ), array(), '', true);

		// Bootstrap JS script
		//if ( is_page( array('edit-client-experience-post') ) ) { wp_enqueue_script('bootstrap5-dbase', esc_url( plugin_dir_url( __FILE__ ) . 'libraries/bootstrap/js/bootstrap.min.js' ), array(), '', true); }
...
// 3rd party plugin - DBase customization files
...
//add_shortcode('dbase_delete_duplicate_pages', 'dbase_delete_duplicate_pages'); 
/*function dbase_delete_duplicate_pages( $atts ) {
...
function dbase_return_business_categories_to_um_plugin() {
...
function wpdocs_theme_name_scripts() {
    wp_enqueue_style('bootstrap5-dbase', plugin_dir_url( __FILE__ ) . 'libraries/bootstrap/css/bootstrap.min.css' ); // Bootstrap 5 css - in header
    wp_enqueue_script('bootstrap5-dbase', esc_url( plugin_dir_url( __FILE__ ) . 'libraries/bootstrap/js/bootstrap.min.js' ), array(), '', true);	// Bootstrap 5 js - in footer
}



// Load language translation capability
add_action( 'init', 'dbase_wpdocs_load_textdomain' );
function dbase_wpdocs_load_textdomain() {
    load_plugin_textdomain( 'd-base-geodirectory-custom-functions', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' ); 
}

// Display custom hamburger menu icon if menu item created is named "hamburgermenu"
add_filter( 'wp_nav_menu_objects' , 'dbase_wp_nav_menu_objects' , 10, 2 );
function dbase_wp_nav_menu_objects( $navitems, $args ) {
	...
	foreach ( $navitems as $val ) {
		if ( $val->post_name == 'hamburgermenu' )	{
			...
			<i class="fa fa-bars custom-hamburger-menu" aria-hidden="true"></i>', 'd-base-geodirectory-custom-functions');
		}
	}
    return $navitems;
}
...
// ************************
//add_shortcode('dbase_admin_find_keyword', 'dbase_admin_find_keyword'); 
function dbase_admin_find_keyword( $atts ) {
...
add_action( 'wp_head', function(){
	...
	<style>
		...
		/* Dbase plugin */
		button.dbase-button {background-color:#ed3439;}
...
//add_shortcode('dbase_random_posts', 'dbase_random_posts_order'); 
function dbase_random_posts_order( $atts ) {
